    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs bg-violet-900 text-violet-300 px-2 py-0.5 rounded-full">
          {verification.privacyMode === "mpc" ? "Arcium MPC" : "Mock mode"}
        </span>
      </div>

      <div className="grid grid-cols-1 gap-3">
        <Field label="payment_valid" value={String(verification.payment_valid)} />
        <Field label="sender_wallet" value={verification.sender} hidden />
      ...
      <div className="mt-2 p-3 bg-violet-950 rounded-lg border border-violet-800">
        <p className="text-xs text-violet-400 uppercase tracking-wide mb-1">
          Arcium Computation Proof
        </p>
        <p className="font-mono text-xs text-violet-300 break-all">
          {verification.arciumProof}
        </p>
      </div>

      {/* What's hidden */}
      <div className="p-3 bg-slate-800 rounded-lg">
        <p className="text-xs text-slate-400 mb-2">Hidden by Arcium MPC cluster:</p>
        <ul className="space-y-1">
          {["sender wallet address", "exact payment amount", "API endpoint accessed", "access frequency per wallet"].map((item) => (
